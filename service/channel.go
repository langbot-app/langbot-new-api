...
func DisableChannel(channelError types.ChannelError, reason string) {
	common.SysLog(fmt.Sprintf("channel '%s' (#%d) encountered error, preparing to disable, reason: %s", channelError.ChannelName, channelError.ChannelId, reason))

	// Check if auto-disable feature is enabled
	if !channelError.AutoBan {
		common.SysLog(fmt.Sprintf("channel '%s' (#%d) auto-disable not enabled, skipping disable operation", channelError.ChannelName, channelError.ChannelId))
		return
	}
