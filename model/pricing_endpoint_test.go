...
func TestPricingIncludesLangBotModelMetadata(t *testing.T) {
	resetPricingEndpointTestTables(t)

	insertPricingEndpointChannel(t, 205, constant.ChannelTypeOpenAI, dto.ChannelOtherSettings{})
	insertPricingEndpointAbility(t, 205, "langbot-space-model")
	modelMeta := &Model{
		ModelName:     "langbot-space-model",
		...
		FeaturedOrder: 7,
		Category:      "chat",
		LLMAbilities:  `["vision","function_call"]`,
		NameRule:      NameRuleExact,
	}
	...
	pricing, ok := pricingByName["langbot-space-model"]
	require.True(t, ok)
	assert.Equal(t, modelMeta.UUID, pricing.UUID)
	assert.NotEmpty(t, pricing.UUID)
